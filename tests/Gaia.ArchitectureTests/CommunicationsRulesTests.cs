...
public sealed class CommunicationsRulesTests
{
 ...
 [Theory][InlineData(1,"publish",true)][InlineData(2,"finish",true)][InlineData(3,"publish",true)][InlineData(4,"publish",false)]public void EventTransitionsAreControlled(int state,string action,bool expected)=>Assert.Equal(expected,CommunicationsRules.EventTransition(state,action));
