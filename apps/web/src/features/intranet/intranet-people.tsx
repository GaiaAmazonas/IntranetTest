...
export function IntranetPeople() {
  ...
  useEffect(()=>{let active=true;const path=peoplePath(page,pageSize,query,selectedUnitId,includeDescendants),cached=peopleCache.get(path);Promise.resolve().then(()=>{if(!active)return;if(cached){setResult(cached);setLoading(false);}else setLoading(true);setError("");});apiRequest<PeoplePage>(path).then(data=>{peopleCache.set(path,data);if(active)setResult(data);}).catch(reason=>{if(active&&!cached)setError(reason instanceof Error?reason.message:"No fue posible cargar Personas.");}).finally(()=>{if(active)setLoading(false);});return()=>{active=false;};},[includeDescendants,page,query,selectedUnitId]);
