  ...
  return <div className="mt-6">
    ...
    {view === "explore" ? <div className="mt-4 grid min-h-[610px] overflow-hidden rounded-2xl border border-[#dfe7dc] bg-white lg:grid-cols-[300px_minmax(0,1fr)]">
      <aside className="border-b border-[#dfe7dc] bg-[#f7f9f6] lg:border-b-0 lg:border-r">
        ...
        <div className="max-h-[540px] overflow-y-auto p-2" role="tree"><button className={`mb-1 flex min-h-11 w-full items-center gap-2 rounded-xl px-3 text-left text-[12px] font-bold ${!activeUnitId?"bg-white text-[#174b35] shadow-sm ring-1 ring-[#cbd9c7]":"text-[#52685e] hover:bg-white"}`} onClick={()=>setSelectedUnitId("")} type="button"><Users size={15}/>Toda la organización</button>{treeRows(children, collapsed).map(({unit,depth,hasChildren}) => <div className={`flex min-h-11 items-center rounded-xl ${unit.id===activeUnitId?"bg-white shadow-sm ring-1 ring-[#cbd9c7]":"hover:bg-white"}`} key={unit.id} style={{paddingLeft:`${8+depth*15}px`}}>
          ...
          <button className="min-w-0 flex-1 py-2 pr-2 text-left" onClick={()=>setSelectedUnitId(unit.id)} type="button"><strong className="block truncate text-[12px] text-[#173b2c]">{unit.name}</strong><small className="mt-0.5 flex items-center gap-2 text-[9px] text-[#738178]"><span>{unit.code}</span><span>{countByUnit.get(unit.id)?.size??0} personas</span></small></button>
        </div>)}</div>
      </aside>
