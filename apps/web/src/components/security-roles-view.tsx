...
export function SecurityRolesView({ roles, permissions, modules, onReload, onPermissionsPublished }: { roles: SecurityRole[]; permissions: SecurityPermission[]; modules: SecurityModule[]; onReload: () => Promise<void>; onPermissionsPublished: () => Promise<void> }) {
  ...
  const grouped = useMemo(() => {
    ...
    for (const permission of permissions) {
      ...
      const parent = direct.parentId ? moduleMap.get(direct.parentId) : undefined;
      const visualModule = parent?.code.toUpperCase() === "INT.APLICACIONES" ? parent : direct;
      const children = result.get(root.id)!; if (!children.has(visualModule.id)) children.set(visualModule.id, []); children.get(visualModule.id)!.push(permission);
    }
  ...
  function setPermissionGroup(groupPermissions: SecurityPermission[], enable: boolean) { if (!selected) return; const next = new Set(selectedCodes); for (const permission of groupPermissions) { if (enable) next.add(permission.code); else next.delete(permission.code); } setDraft(next); setDraftRoleId(selected.id); }
  ...
  async function savePermissions() {
    ...
    try {
      ...
      setConfirmPermissions(false); setDraft(null); setDraftRoleId(null);
      await Promise.all([onReload(), onPermissionsPublished()]);
  ...
  async function saveRole() {
    ...
    try {
      ...
      const savedRoleId = await apiRequest<string>(roleForm.id ? `/api/security/roles/${roleForm.id}` : "/api/security/roles", { method: roleForm.id ? "PUT" : "POST", body });
      ...
      setSelectedId(savedRoleId); setDraft(null); setDraftRoleId(null); setRoleForm(null); await onReload();
  ...
  return <div className="gaia-roles-workspace">
    ...
    <section className="gaia-role-detail" aria-live="polite">
      {!selected ? <EmptyState title="No hay roles configurados" description="Crea el primer rol para comenzar." /> : <>
        ...
        <div className="gaia-permission-tree">{grouped.map(([rootId, children]) => {
          ...
          return <section key={rootId}><div className="gaia-permission-root"><button aria-expanded={!closed} className="gaia-permission-expand" onClick={() => setCollapsed(current => { const next = new Set(current); if (next.has(rootId)) next.delete(rootId); else next.add(rootId); return next; })} type="button"><ChevronDown className={closed ? "is-closed" : ""} size={18} /><span><strong>{root.name}</strong><small>{root.description}</small></span><span className={`gaia-permission-count ${selectedCount===rows.length?"is-complete":selectedCount?"is-partial":"is-empty"}`}>{selectedCount}/{rows.length}</span></button><button className="gaia-permission-bulk" onClick={() => setPermissionGroup(rows, selectedCount !== rows.length)} type="button">{selectedCount === rows.length ? "Quitar todo" : "Seleccionar todo"}</button></div>{!closed && <div className="gaia-permission-children">{[...children.entries()].sort(([a], [b]) => (moduleMap.get(a)?.order ?? 0) - (moduleMap.get(b)?.order ?? 0)).map(([moduleId, modulePermissions]) => {
            const featureModule = moduleMap.get(moduleId)!;
            const isApplicationGroup=featureModule.code.toUpperCase()==="INT.APLICACIONES";const featureClosed = collapsed.has(moduleId); const featureSelected = modulePermissions.filter(permission => selectedCodes.has(permission.code)).length; return <article key={moduleId}><div className="gaia-permission-feature-toggle"><button aria-expanded={!featureClosed} className="gaia-permission-expand" onClick={() => setCollapsed(current => { const next = new Set(current); if (next.has(moduleId)) next.delete(moduleId); else next.add(moduleId); return next; })} type="button"><ChevronDown className={featureClosed ? "is-closed" : ""} size={16} /><span><strong>{featureModule.name}</strong>{featureModule.id !== root.id && <small>{featureModule.type}</small>}</span><span className={`gaia-permission-count ${featureSelected===modulePermissions.length?"is-complete":featureSelected?"is-partial":"is-empty"}`}>{featureSelected}/{modulePermissions.length}</span></button><button className="gaia-permission-bulk" onClick={() => setPermissionGroup(modulePermissions, featureSelected !== modulePermissions.length)} type="button">{featureSelected === modulePermissions.length ? "Quitar" : "Seleccionar"}</button></div>{!featureClosed && <div className={`gaia-permission-actions${isApplicationGroup?" is-applications":""}`}>{modulePermissions.map(permission => <label className={selectedCodes.has(permission.code) ? "is-checked" : ""} key={permission.id}><input checked={selectedCodes.has(permission.code)} onChange={() => togglePermission(permission)} type="checkbox" /><span><strong>{isApplicationGroup?functionalPermissionName(permission):actionLabel(permission.action)}</strong>{!isApplicationGroup&&<small>{functionalPermissionName(permission)}</small>}{isApplicationGroup&&<small>Acceso a la aplicación</small>}{technical && <code>{permission.code}</code>}</span></label>)}</div>}</article>;
          })}</div>}</section>;
        })}</div>
    ...
    <ConfirmDialog confirmLabel="Guardar permisos" description={selected ? `Rol destino: ${selected.name} (${selected.code}). Se reemplazará únicamente la configuración explícita de permisos de este rol; las asignaciones de usuarios no se modificarán.` : ""} loading={saving} onCancel={() => setConfirmPermissions(false)} onConfirm={() => void savePermissions()} open={confirmPermissions} title="¿Confirmar cambios de permisos?" />
