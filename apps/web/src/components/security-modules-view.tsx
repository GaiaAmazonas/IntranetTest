...
export function SecurityModulesView({ modules, onAccessChanged, onReload }: { modules: SecurityModule[]; onAccessChanged: () => Promise<void>; onReload: () => Promise<void> }) {
  ...
  async function saveModule() {
    ...
    try {
      ...
      if (route && !isValidRoute(route)) throw new Error("La ruta debe comenzar por / o ser un enlace completo HTTP/HTTPS.");
      if (code.toUpperCase().startsWith("INT.APP.") && !route) throw new Error("La aplicación necesita una ruta o enlace.");
      await apiRequest(form.id ? `/api/security/modules/${form.id}` : "/api/security/modules", { method: form.id ? "PUT" : "POST", body: JSON.stringify({ code, name: form.name, description: form.description || null, type: form.type, parentId: form.parentId || null, route: route || null, icon: form.icon || null, order: form.order, visible: form.visible, isActive: form.isActive }) });
