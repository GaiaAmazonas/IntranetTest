    ...
    ICollaboratorEmailStore emailStore, ICollaboratorPhoneStore phoneStore) : IAdministrativePersonnelImporter
{
    ...
    {
        var counts=new ContactCounts();foreach(var row in sheet.Rows){var doc=Document(row.Get("Número de documento"));var candidates=EmailCandidates(row).ToArray();if(excluded.Contains(doc)){counts.Omitted+=candidates.Length;continue;}if(!parties.TryGetValue(doc,out var party)){counts.Errors+=candidates.Length;continue;}var current=await emailStore.ListAsync(party.Id,token);foreach(var item in candidates){var found=current.FirstOrDefault(x=>NormalizeEmail(x.Email)==NormalizeEmail(item.Value));if(found is not null){if(found.IsPrimary!=item.Primary||found.ContactType!=item.ContactType||!found.IsActive){var result=await emailStore.UpdateAsync(party.Id,found.Id,new(NormalizeEmail(found.Email),found.Observations,item.Primary,true,"administrative-import",item.ContactType),token);if(result.Status==RelatedWriteStatus.Updated)counts.Updated++;else counts.Errors++;}else counts.Existing++;continue;}var created=await emailStore.CreateAsync(party.Id,new(NormalizeEmail(item.Value),null,item.Primary,true,"administrative-import",item.ContactType),token);if(created.Status==RelatedWriteStatus.Created)counts.Created++;else{counts.Errors++;issues.Add(new(row.Number,sheet.Name,"EMAIL_NOT_CREATED",$"{item.Value}: {created.Status}"));}}}return counts;
    }
    ...
    {
        var counts=new ContactCounts();foreach(var row in sheet.Rows){var doc=Document(row.Get("Número de documento"));var candidates=PhoneCandidates(row).ToArray();if(excluded.Contains(doc)){counts.Omitted+=candidates.Length;continue;}if(!parties.TryGetValue(doc,out var party)){counts.Errors+=candidates.Length;continue;}var current=await phoneStore.ListAsync(party.Id,token);foreach(var item in candidates){var found=current.FirstOrDefault(x=>NormalizePhone(x.Number)==NormalizePhone(item.Value));if(found is not null){if(found.IsPrimary!=item.Primary||found.ContactType!=item.ContactType||!found.IsActive||!found.PhoneType.Equals("CELULAR",StringComparison.OrdinalIgnoreCase)){var result=await phoneStore.UpdateAsync(party.Id,found.Id,new(NormalizePhone(found.Number),found.Extension,found.Observations,item.Primary,"CELULAR",true,"administrative-import",item.ContactType),token);if(result.Status==RelatedWriteStatus.Updated)counts.Updated++;else counts.Errors++;}else counts.Existing++;continue;}var created=await phoneStore.CreateAsync(party.Id,new(NormalizePhone(item.Value),null,null,item.Primary,"CELULAR",true,"administrative-import",item.ContactType),token);if(created.Status==RelatedWriteStatus.Created)counts.Created++;else{counts.Errors++;issues.Add(new(row.Number,sheet.Name,"PHONE_NOT_CREATED",$"{item.Value}: {created.Status}"));}}}return counts;
    }

    private static IEnumerable<(string Value,bool Primary,int ContactType)> EmailCandidates(WorkbookRow row){var institutional=PersonnelWorkbookReader.Optional(row.Get("Correo institucional"));var personal=PersonnelWorkbookReader.Optional(row.Get("Correo personal"));return new[]{(Value:institutional,Primary:true,ContactType:2),(Value:personal,Primary:institutional is null,ContactType:1)}.Where(x=>x.Value is not null).GroupBy(x=>NormalizeEmail(x.Value!),StringComparer.OrdinalIgnoreCase).Select(x=>(x.Key,x.Any(y=>y.Primary),x.Max(y=>y.ContactType)));}
    private static IEnumerable<(string Value,bool Primary,int ContactType)> PhoneCandidates(WorkbookRow row){var personal=PersonnelWorkbookReader.Optional(row.Get("Celular personal"));var corporate=PersonnelWorkbookReader.Optional(row.Get("Celular corporativo"));return new[]{(Value:personal,Primary:corporate is null,ContactType:1),(Value:corporate,Primary:true,ContactType:2)}.Where(x=>x.Value is not null).GroupBy(x=>NormalizePhone(x.Value!),StringComparer.OrdinalIgnoreCase).Select(x=>(x.Key,x.Any(y=>y.Primary),x.Max(y=>y.ContactType)));}
