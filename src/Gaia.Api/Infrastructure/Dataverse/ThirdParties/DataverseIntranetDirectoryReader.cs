    ...
    : IIntranetDirectoryReader
{
        ...
        bool includeDescendants, int page, int pageSize, CancellationToken token)
    {
        ...
        // Keep one deterministic pagination path for the complete directory and
        // filtered searches. Dataverse next links can restart at the first page
        // when this query is combined with its server-driven page-size preference.
        // Paging after the directory enrichment also guarantees consistent results.
        var rows = await DataverseJson.ReadAllAsync(client,
            $"{thirdParty.EntitySetName}?$select={thirdParty.PrimaryIdAttribute},{name}&$filter=statecode eq 0&$orderby={name},{thirdParty.PrimaryIdAttribute}", token);
