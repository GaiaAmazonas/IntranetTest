...
internal sealed class DataverseCollaboratorEmailStore(IDataverseDelegatedClientFactory factory) : ICollaboratorEmailStore
{
    ...
    public async Task<IReadOnlyList<CollaboratorEmailResponse>> ListAsync(Guid parent, CancellationToken token)
    {
        ...
        return rows.Select(x => new CollaboratorEmailResponse(GuidValue(x, fields.Id), StringValue(x, fields.Email) ?? "", StringValue(x, fields.Notes),
            BoolValue(x, fields.Primary), (DataverseJson.OptionalInt32(x, "statecode") ?? 0) == 0)).ToArray();
    }
    ...
    private async Task<RelatedWriteResult> WriteCore(Guid parent, Guid? id, CollaboratorEmailCommand command, CancellationToken token)
    {
        ...
        var payload = new Dictionary<string, object?> { [f.Email] = command.Email, [f.Notes] = command.Observations,
            [f.Primary] = command.IsPrimary, ["statecode"] = command.IsActive ? 0 : 1,
    ...
    private sealed record EmailFields(string Id, string Email, string Notes, string Primary, string Parent)
    { public string Select => string.Join(',', Id, Email, Notes, Primary, $"_{Parent}_value", "statecode"); public static EmailFields From(DataverseTableMetadata m) => new(m.PrimaryIdAttribute, m.Attribute("gaia_Correoelectronico"), m.Attribute("gaia_Observaciones"), m.Attribute("gaia_Principal"), m.Attribute("gaia_Tercero")); }
...
internal sealed class DataverseCollaboratorPhoneStore(IDataverseDelegatedClientFactory factory) : ICollaboratorPhoneStore
{
    ...
    { var (client, table, f) = await Context(token); var rows = await DataverseJson.ReadAllAsync(client, $"{table.EntitySetName}?$select={f.Select}&$filter=_{f.Parent}_value eq {parent:D}&$orderby={f.Number}", token); var types = (await DataverseMetadataResolver.ChoicesAsync(client, Table, f.Type, token)).ToDictionary(x => x.Value, x => x.Key); return rows.Select(x => { var value = DataverseJson.OptionalInt32(x, f.Type); var label = DataverseCollaboratorEmailStore.StringValue(x, $"{f.Type}@OData.Community.Display.V1.FormattedValue") ?? (value.HasValue && types.TryGetValue(value.Value, out var mapped) ? mapped : ""); return new CollaboratorPhoneResponse(DataverseCollaboratorEmailStore.GuidValue(x, f.Id), DataverseCollaboratorEmailStore.StringValue(x, f.Number) ?? "", DataverseCollaboratorEmailStore.StringValue(x, f.Extension), DataverseCollaboratorEmailStore.StringValue(x, f.Notes), DataverseCollaboratorEmailStore.BoolValue(x, f.Primary), label, (DataverseJson.OptionalInt32(x, "statecode") ?? 0) == 0); }).ToArray(); }
    ...
    private async Task<RelatedWriteResult> WriteCore(Guid parent, Guid? id, CollaboratorPhoneCommand command, CancellationToken token)
    {
        ...
        var choices = await DataverseMetadataResolver.ChoicesAsync(client, Table, f.Type, token); if (!choices.TryGetValue(command.PhoneType, out var typeValue)) return new(RelatedWriteStatus.InvalidOption);
    ...
    private sealed record PhoneFields(string Id, string Number, string Extension, string Notes, string Primary, string Parent, string Type)
    { public string Select => string.Join(',', Id, Number, Extension, Notes, Primary, $"_{Parent}_value", Type, "statecode"); public static PhoneFields From(DataverseTableMetadata m) => new(m.PrimaryIdAttribute, m.Attribute("gaia_Numero"), m.Attribute("gaia_Extension"), m.Attribute("gaia_Observaciones"), m.Attribute("gaia_Principal"), m.Attribute("gaia_Tercero"), m.Attribute("gaia_Tipodetelefono")); }
}
